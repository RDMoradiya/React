    ...
    <div className="container">
      <div className="row">
        <div className="col-md-10">
          <h1>Top Movies List :- {enteredMovieData.length}</h1>
        </div>
        <div className="col-md-2">
          <button className="btn btn-primary mb-2" onClick={hideHandler}>
            Hide
          </button>
          <button className="btn btn-success mb-2 ms-2" onClick={showHandler}>
            Show
          </button>
        </div>
      </div>
              ...
              {enteredMovieData.map((item, index) => {
                return (
                  <Main
                    ...
                    link={item.link}
                    onClick={() => watchListHandler(item.MovieDetails)}
                  />
                );
              })}
            </div>
          ...
          <div className="col-2">
            ...
            <ul className="list-group">
              {EnterdData.map((item, index) => {
                return <Info key={index} label={item} />;
              })}
            </ul>
          </div>
        </div>
